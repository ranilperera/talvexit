...
function StatCardSkeleton() {
  return (
    <div className="rounded-lg bg-slate-900 p-5 shadow-sm border-t-4 border-slate-800">
...
function LastUpdated({ timestamp }: { timestamp: string }) {
  ...
  return <span className="text-xs text-slate-500">Last updated {label}</span>;
}
        ...
        <tbody className="divide-y divide-gray-100">
          {Object.entries(queues).map(([name, q]) => (
            <tr key={name}>
              ...
              <td className="px-4 py-3 text-slate-400">{q.waiting}</td>
              <td className="px-4 py-3 text-slate-400">{q.active}</td>
              <td
                className={
              ...
              <td className="px-4 py-3 text-slate-400">{q.delayed}</td>
            </tr>
          ))}
        </tbody>
      </table>
...
function SystemStatusRow({ report }: { report: HealthReport }) {
  ...
  return (
    <div className="grid grid-cols-3 gap-4">
      <div className="flex items-start gap-3 rounded-lg bg-slate-900 border border-slate-800 p-4 shadow-sm">
        <StatusDot healthy={db.is_healthy} />
        <div>
          <p className="text-sm font-semibold text-slate-300">Database</p>
      ...
      <div className="flex items-start gap-3 rounded-lg bg-slate-900 border border-slate-800 p-4 shadow-sm">
        ...
        <div>
          <p className="text-sm font-semibold text-slate-300">Kubernetes</p>
          {k8s.note ? (
            <p className="mt-0.5 text-xs text-slate-500">{k8s.note}</p>
          ) : (
            <p className="mt-0.5 text-xs text-slate-500">
      ...
      <div className="flex items-start gap-3 rounded-lg bg-slate-900 border border-slate-800 p-4 shadow-sm">
        ...
        <div>
          <p className="text-sm font-semibold text-slate-300">App Insights</p>
          {ai.note ? (
            <p className="mt-0.5 text-xs text-slate-500">{ai.note}</p>
          ) : (
            <p className="mt-0.5 text-xs text-slate-500">{ai.failed_requests_1h} failed req/hr</p>
          )}
        </div>
      </div>
    </div>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <h2 className="text-lg font-semibold text-slate-200">Platform Overview</h2>
        ...
        {healthQuery.isLoading ? (
          ...
          <>
            ...
            <StatCard
              ...
              value={m?.pending_companies ?? 0}
              color={m && m.pending_companies > 0 ? 'yellow' : 'green'}
