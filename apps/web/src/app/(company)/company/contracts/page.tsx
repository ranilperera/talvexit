...
export default function CompanyContractsPage() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['company-contracts', companyData],
    queryFn: () =>
      companyApi
        .get<{ success: boolean; data: { contracts: ContractSummary[] } }>(
          `/api/v1/provider/tender-contracts?company_id=${companyData!}`,
        )
  ...
  return (
    <PageContainer className="space-y-8">
      <div>
        ...
        <p className="mt-1 text-sm text-slate-500">Tender contracts awarded to your company.</p>
      </div>
      ...
      {active.length > 0 && (
        <section>
          ...
          <div className="space-y-3">
            {active.map((c) => <ContractCard key={c.id} contract={c} {...(companyData ? { companyId: companyData } : {})} />)}
          </div>
        </section>
      )}
      ...
      {closed.length > 0 && (
        <section>
          ...
          <div className="space-y-3">
            {closed.map((c) => <ContractCard key={c.id} contract={c} {...(companyData ? { companyId: companyData } : {})} />)}
          </div>
        </section>
      )}
    </PageContainer>
...
function ContractCard({ contract: c, companyId }: { contract: ContractSummary; companyId?: string }) {
  ...
  const href = companyId
    ? `/company/contracts/${c.id}?company_id=${companyId}`
    : `/company/contracts/${c.id}`;
