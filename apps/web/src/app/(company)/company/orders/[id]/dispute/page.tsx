...
import { X, Upload, FileText, AlertTriangle, UserX, ClipboardX, Eye } from 'lucide-react';
...
// ─── Grounds available to suppliers/contractors ───────────────────────────────
// CUSTOMER_WITHHOLDING_APPROVAL is the primary supplier ground.
// Others are included for completeness (e.g. data_breach if customer misuses creds).

const GROUNDS: GroundOption[] = [
  {
    key: 'CUSTOMER_WITHHOLDING_APPROVAL',
    label: 'Customer Withholding Approval',
    description: 'Work is complete and meets scope but customer refuses to approve',
    Icon: ClipboardX,
  },
  {
    key: 'SCOPE_MISREPRESENTATION',
    label: 'Scope Misrepresentation',
    description: 'The agreed scope was materially misrepresented by the customer',
    Icon: Eye,
  },
  {
    key: 'ACCESS_EXCEEDED',
    label: 'Access / Security Issue',
    description: 'Customer provided false access details or created a security risk',
    Icon: UserX,
  },
];
...
export default function CompanyRaiseDisputePage() {
  ...
  async function handleSubmit() {
    setSubmitting(true);
    try {
      const res = await customerApi.post<{ success: boolean; data: { dispute: { id: string } } }>(
      ...
      toast.success('Dispute filed. An admin will review within 4 hours.');
      router.push('/company/disputes/' + res.data.data.dispute.id);
    } catch {
  ...
  return (
    <PageContainer className="space-y-6">
      ...
      {/* Header */}
      <div>
        <Link href={`/company/orders/${orderId}`} className="text-sm text-slate-400 hover:text-slate-200 transition-colors no-underline flex items-center gap-1 mb-4">
          ← Return to Order
        </Link>
        <h1 className="font-display font-bold text-2xl text-slate-100">Raise a Dispute</h1>
        <p className="text-sm text-slate-400 mt-1">As a supplier, you can raise a dispute if the customer is withholding approval on completed work.</p>
      </div>
      ...
      <div className="bg-amber-500/10 border border-amber-500/30 rounded-2xl px-5 py-4 flex gap-3">
        ...
        <p className="text-sm text-amber-300 leading-relaxed">
          <strong>Disputes pause the order</strong> and escrow is held until admin resolution.
          Please attempt direct resolution first.
        </p>
      </div>
      ...
      {order && (
        <div className="bg-slate-900 border border-slate-800 rounded-2xl px-5 py-4">
          <h2 className="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-3">Order</h2>
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 text-sm">
            ...
            <div>
              ...
              <p className="text-slate-200">{order.customer?.full_name ?? '—'}</p>
            </div>
            <div>
      ...
      <div className="space-y-2">
        ...
        <textarea
          ...
          placeholder="Describe the dispute in detail. Include dates, deliverables completed, and any communication with the customer..."
          className={clsx(
      ...
      <div className="space-y-3">
        <label className="text-xs font-medium text-slate-400 tracking-wide block">
          Evidence <span className="text-slate-500">(optional — screenshots, logs, emails, up to 10 files)</span>
        </label>
        ...
        >
          ...
          <p className="text-xs text-slate-600 mt-1">Images, PDFs, logs — max 20 MB each</p>
          <input
            ref={fileRef}
            type="file"
            multiple
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-2xl px-5 py-4 space-y-4">
        <label className="flex items-start gap-3 cursor-pointer">
          ...
          <span className="text-sm text-slate-300 leading-relaxed">
            I confirm this dispute is filed in good faith and the information above is accurate to the best of my knowledge.
          </span>
        </label>
            ...
            <Dialog.Content className="fixed inset-0 z-50 flex items-center justify-center p-4">
              <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 max-w-sm w-full space-y-4">
                ...
                <Dialog.Description className="text-sm text-slate-400 leading-relaxed">
                  The order will be paused and an admin will be assigned within 4 hours to review all evidence. This action cannot be undone.
                </Dialog.Description>
