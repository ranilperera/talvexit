...
const OUTCOME_CFG: Record<string, { label: string; color: Color; detail: string }> = {
  FULL_PAYMENT:    { label: 'Full Payment to You',      color: 'green', detail: 'Full payment released to your account.' },
  FULL_REFUND:     { label: 'Full Refund to Customer',  color: 'red',   detail: 'Full amount refunded to the customer. No payout.' },
  PARTIAL_PAYMENT: { label: 'Partial Resolution',       color: 'amber', detail: 'Partial payment to you; remainder refunded to customer.' },
  REMEDY_REQUIRED: { label: 'Remedy Required',          color: 'blue',  detail: 'You must provide a remedy. Order remains open.' },
};
...
function partyLabel(role: string, myRole: string): string {
  if (role === 'CONTRACTOR') return myRole === 'CONTRACTOR' ? 'You (Expert)' : 'Expert';
  return myRole === 'CUSTOMER' ? 'You (Customer)' : 'Customer';
}

// ─── Page ─────────────────────────────────────────────────────────────────────

export default function CompanyDisputeDetailPage() {
  ...
  async function uploadFiles(incoming: FileList) {
    if (subFiles.length + incoming.length > 10) { toast.error('Maximum 10 files allowed'); return; }
  ...
  async function submitEvidence() {
    if (subDesc.trim().length < 10) { toast.error('Please add a description (min 10 chars)'); return; }
    ...
    try {
      ...
      setSubFiles([]);
      refresh();
    } catch { toast.error('Failed to submit evidence'); }
  ...
  if (!dispute) {
    return <div className="max-w-3xl mx-auto px-4 py-20 text-center"><p className="text-slate-400">Dispute not found.</p></div>;
  }
  ...
  const timelineSteps = [
    { label: `Dispute filed by ${partyLabel(dispute.filed_by_role, myRole)}`, time: dispute.filed_at, done: true },
    { label: dispute.assigned_at ? `Assigned to admin ${dispute.assigned_admin_name ?? ''}` : 'Awaiting admin assignment', time: dispute.assigned_at ?? null, done: !!dispute.assigned_at },
    { label: 'Under review', time: null, done: ['UNDER_REVIEW', 'DETERMINED', 'CLOSED'].includes(dispute.status) },
    { label: isDetermined ? 'Determination issued' : 'Determination pending', time: dispute.determination?.determined_at ?? null, done: isDetermined },
  ];
      ...
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          <p className="text-xs text-slate-500 mb-1">
            <Link href={`/company/orders/${dispute.order_id}`} className="hover:text-slate-300 transition-colors no-underline">
              ← Back to Order
            </Link>
          </p>
          <h1 className="font-display font-bold text-2xl text-slate-100">Dispute #{shortId}</h1>
        ...
        <aside className="space-y-4 lg:sticky lg:top-6">
              ...
              {timelineSteps.map((step, i) => (
                <li key={i} className="flex gap-3">
                  <div className="flex flex-col items-center">
                    <div className={clsx('w-2.5 h-2.5 rounded-full mt-0.5 shrink-0', step.done ? 'bg-teal-500' : 'bg-slate-700 border border-slate-600')} />
                    {i < timelineSteps.length - 1 && <div className="w-px flex-1 bg-slate-800 mt-1 min-h-[16px]" />}
                  </div>
                  <div className="pb-2">
                    <p className={clsx('text-xs leading-relaxed', step.done ? 'text-slate-300' : 'text-slate-600')}>{step.label}</p>
                    {step.time && <p className="text-xs text-slate-600 mt-0.5">{format(new Date(step.time), 'd MMM yyyy HH:mm')}</p>}
                  </div>
                </li>
              ))}
            </ol>
          ...
          {dispute.evidence_window_open && dispute.evidence_window_closes_at ? (
            <div className="bg-teal-500/10 border border-teal-500/30 rounded-2xl px-5 py-4 space-y-1.5">
              ...
              <p className="text-xs text-teal-400/80">{hoursRemaining(dispute.evidence_window_closes_at)}h remaining</p>
              <p className="text-xs text-slate-600">Closes {format(new Date(dispute.evidence_window_closes_at), 'd MMM yyyy HH:mm')}</p>
            </div>
          ) : !isDetermined ? (
            <div className="bg-slate-900 border border-slate-800 rounded-2xl px-5 py-4">
              ...
              <p className="text-xs text-slate-600">72h submission period has passed.</p>
            </div>
          ) : null}
          ...
          {!isDetermined && (
            <div className="bg-slate-900 border border-slate-800 rounded-2xl px-5 py-4 space-y-2">
              ...
              <ul className="text-xs text-slate-400 space-y-1.5 list-disc list-inside">
                <li>Admin reviews all submissions</li>
                <li>Determination issued within 5 business days</li>
                <li>Payment adjusted automatically per outcome</li>
              </ul>
            </div>
          )}
        </aside>
        ...
        <main className="space-y-6 min-w-0">
          ...
          <section className="bg-slate-900 border border-slate-800 rounded-2xl p-5 space-y-4">
            <div className="flex items-center justify-between">
              <h2 className="font-display font-semibold text-slate-100">
                {dispute.filed_by_role === myRole ? 'Your Filing' : 'Customer Filing'}
              </h2>
              <span className={clsx(
                'text-xs font-medium px-2 py-0.5 rounded-full',
                dispute.filed_by_role === 'CONTRACTOR' ? 'bg-teal-500/15 text-teal-400' : 'bg-blue-500/15 text-blue-400',
              )}>
                {partyLabel(dispute.filed_by_role, myRole)}
              </span>
            </div>
                ...
                return (
                  <div key={sub.id} className={clsx(
                    ...
                    <div className="flex items-center justify-between flex-wrap gap-2">
                      <span className={clsx(
                        'text-xs font-medium px-2 py-0.5 rounded-full',
                        sub.party === 'CONTRACTOR' ? 'bg-teal-500/15 text-teal-400' : 'bg-blue-500/15 text-blue-400',
                      )}>
                        {partyLabel(sub.party, myRole)}
                      </span>
                      <span className="text-xs text-slate-500">{format(new Date(sub.created_at), 'd MMM yyyy HH:mm')}</span>
                    </div>
          ...
          {dispute.evidence_window_open && !isDetermined && (
            <section className="bg-slate-900 border border-slate-800 rounded-2xl p-5 space-y-4">
              ...
              <p className="text-xs text-slate-500">Submit work logs, deliverable screenshots, or communication records during the 72h window.</p>
              ...
              <div className="space-y-2">
                ...
                <textarea
                  ...
                  placeholder="Describe what you're submitting and why it supports your position..."
              ...
              <Button fullWidth loading={subSubmitting} disabled={subDesc.trim().length < 10} onClick={() => { void submitEvidence(); }}>
                Submit Evidence
              </Button>
            </section>
          )}
