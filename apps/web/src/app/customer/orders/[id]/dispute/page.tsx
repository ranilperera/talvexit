...
export default function DisputePage() {
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      router.push('/customer/disputes/' + res.data.data.id);
    } catch {
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 sm:px-6 py-10 space-y-6">
      ...
      {/* Header */}
      <div>
        <Link href={`/customer/orders/${orderId}`} className="text-sm text-slate-400 hover:text-slate-200 transition-colors no-underline flex items-center gap-1 mb-4">
          ← Return to Order
        </Link>
