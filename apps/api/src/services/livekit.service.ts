...
// ─── Startup validation ───────────────────────────────────────────────────────
// Log errors at module load time so they appear in `docker logs onys_api`.

const PLACEHOLDERS = ['', 'YOUR_LIVEKIT_KEY', 'YOUR_LIVEKIT_SECRET', 'CHANGE_ME'];

if (PLACEHOLDERS.includes(process.env.LIVEKIT_API_KEY ?? '')) {
  console.error(
    '[livekit] FATAL: LIVEKIT_API_KEY is not set or is a placeholder. ' +
    'KYC video sessions will fail. Set a real key in .env.prod.',
  );
}
if (PLACEHOLDERS.includes(process.env.LIVEKIT_API_SECRET ?? '')) {
  console.error(
    '[livekit] FATAL: LIVEKIT_API_SECRET is not set or is a placeholder. ' +
    'KYC video sessions will fail. Set a real secret in .env.prod.',
  );
}
if (process.env.NODE_ENV === 'production' && (process.env.LIVEKIT_URL ?? '').includes('localhost')) {
  console.warn(
    `[livekit] WARNING: LIVEKIT_URL="${process.env.LIVEKIT_URL}" contains localhost. ` +
    'Set LIVEKIT_URL to the internal Docker URL (e.g. ws://onys_livekit:7880).',
  );
}
if (process.env.NODE_ENV === 'production' && !process.env.LIVEKIT_PUBLIC_URL) {
  console.warn(
    '[livekit] WARNING: LIVEKIT_PUBLIC_URL is not set. ' +
    'The browser will use LIVEKIT_URL as fallback — this is only correct if there is no TLS-terminating proxy. ' +
    'Set LIVEKIT_PUBLIC_URL to the public WSS URL (e.g. wss://livekit.onsys.online).',
  );
}
