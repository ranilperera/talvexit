...
export class RatingService {
  ...
  async submitRating(
    ...
    // 6. Create rating + update aggregate in a transaction
    if (order.company_id) {
      // Company order — update company aggregate rating only.
      // NOTE: The Rating model requires rated_contractor_id (not nullable).
      // Full company rating rows require a schema migration (add rated_company_id to Rating).
      // For now we update the company aggregate and return a synthetic response.
      const company = await this.prisma.consultingCompany.findUniqueOrThrow({
        where: { id: order.company_id },
        select: { overall_rating: true, rating_count: true, primary_admin_id: true },
      });
      ...
      void writeAudit(this.prisma, {
        ...
        metadata: { overall_score, company_id: order.company_id, submitted_by: customerId },
      });

      // Notify company primary admin
      const adminUser = await this.prisma.user.findUnique({
        where: { id: company.primary_admin_id },
        select: { email: true },
      });
      if (adminUser?.email) {
      ...
      // Return a rating-compatible shape (id = orderId as identifier)
      return { id: orderId, overall_score, order_id: orderId, company_id: order.company_id };
    }
