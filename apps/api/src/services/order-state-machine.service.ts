...
async function guardToPaymentHeld(
  ...
  if (providerType === 'company') {
    ...
    if (!company) {
      return new AppError('COMPANY_NOT_FOUND', 422, 'Company not found');
    }
    const insuranceValid = isCurrentlyValid(company.insurance_certificates);
    if (!insuranceValid) {
      return new AppError(
        'INSURANCE_EXPIRED',
        402,
        'Company insurance has expired. Payment cannot proceed until insurance is renewed.',
      );
    }
    return null;
  }

  // Individual contractor path
...
// ─── isAuthorizedContractorSide ───────────────────────────────────────────────
// Returns true if userId may act as the "contractor side" on an order.
// Works for both individual contractor orders and company-owned orders.

export async function isAuthorizedContractorSide(
  order: {
    contractor_user_id: string | null;
    company_id: string | null;
    executing_member_id: string | null;
  },
  userId: string,
  prisma: PrismaClient,
): Promise<boolean> {
  ...
  if (providerType === 'contractor') {
    return order.contractor_user_id === userId;
  }

  // Company order — executing member OR any COMPANY_ADMIN/SENIOR_CONSULTANT
  if (order.executing_member_id === userId) return true;
  ...
  return (
    ...
    ['COMPANY_ADMIN', 'SENIOR_CONSULTANT'].includes(membership.role)
  );
}
