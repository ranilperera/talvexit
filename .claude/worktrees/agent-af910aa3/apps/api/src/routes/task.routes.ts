...
export async function taskRoutes(
  app: FastifyInstance,
  opts: { taskService: TaskService; smrService: ScopeModificationService },
) {
  const { taskService, smrService } = opts;
  ...
  app.post('/tasks', { preHandler: [authenticate] }, async (req, reply) => {
  ...
  app.post('/tasks/:id/publish', { preHandler: [authenticate] }, async (req, reply) => {
    const { id } = req.params as { id: string };
    try {
