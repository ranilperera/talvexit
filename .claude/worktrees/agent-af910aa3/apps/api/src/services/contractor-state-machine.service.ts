...
import { getRequiredTier, validateCoverageMet } from './insurance-tier.service.js';
...
function guardIncompleteToPending(profile: ContractorProfile): string | null {
  ...
  if (!profile.employment_type)
    return 'Employment declaration required (step 2)';
...
async function guardPendingToActive(
  ...
  // Check insurance_tier_met flag (set by adminReviewCertificate when coverage is complete)
  if (!profile.insurance_tier_met) {
    return 'Insurance coverage not yet verified or does not meet required tier';
  }

  // Double-check live certificate state (in case certs expired since flag was set)
  ...
  const coverage = validateCoverageMet(certs, requiredTier);

  if (!coverage.met) {
    return `Insurance gaps: ${coverage.gaps.join('; ')}`;
  }
...
export function getOnboardingStatus(profile: ContractorProfile): OnboardingStatus {
  const insuranceMet = profile.insurance_tier_met === true;
  ...
  const steps: StepStatus[] = [
    ...
    },
    {
      step: 6,
      name: 'Insurance',
      complete: insuranceMet,
      blocking_reason: insuranceMet
        ? null
        : 'Insurance coverage not yet verified or does not meet required tier',
    },
    {
      step: 7,
